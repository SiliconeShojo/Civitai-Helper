...
def build_article_from_version(version):
    ...
    if download_url:
        ...
        download_btn = templates.update_download_btn.substitute(
    ...
    description_html = ""
    if description:
        description_html = util.safe_html(description)

    media_html = ""
    if img_url:
        if img_url.lower().endswith('.mp4') or img_url.lower().endswith('.webm'):
            media_html = f'<video src="{img_url}" class="ch-update-card-video" autoplay loop muted playsinline></video>'
        else:
            media_html = f'<img src="{img_url}" class="ch-update-card-thumb" loading="lazy">'

    article = templates.update_card.substitute(
...
def check_models_new_version_to_md(model_types:list, progress=gr.Progress()) -> str:
    yield "Searching for new versions..."
    new_versions = civitai.check_models_new_version_by_model_types(model_types, 0.2, progress)

    if not new_versions:
        util.printD("Done: no new versions found.")
        yield "No models have new versions"
        return
    ...
    output = templates.update_card_grid.substitute(cards="".join(articles))
