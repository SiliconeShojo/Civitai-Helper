...
def on_ui_tabs():
    ...
    with gr.Blocks(
        ...
        with gr.Accordion("ℹ️ Get Model Info", open=False, elem_classes="ch_box"):
            sections.get_model_info_by_url_section()

        with gr.Accordion("⬇️ Download Model", open=False, elem_classes="ch_box"):
        ...
        with gr.Accordion("📑 Scan Duplicates", open=False, elem_classes="ch_box"):
            sections.scan_for_duplicates_section()

        with gr.Accordion("🔄 Check Updates", open=False, elem_classes="ch_box"):
...
def on_ui_settings():
    ...
    shared.opts.add_option(
        "ch_nsfw_threshold",
        shared.OptionInfo(
            list(civitai.NSFW_LEVELS.keys())[0],
            util.dedent(
                """
...
util.GRADIO_FALLBACK = not (parse_version(gr.__version__) > parse_version("3.42.0"))
